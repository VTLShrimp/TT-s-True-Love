...
public class BossHealth : MonoBehaviour, IHealth
{
    ...
    private Rigidbody2D rrb; // Adding 'new' keyword to hide inherited member
    ...
    public void ResetBoss()
    {
        ...
        rrb.velocity = Vector2.zero; // Reset velocity to ensure no movement
        rrb.gravityScale = 1; // Restore gravity scale if altered
    }
    ...
    public void TakeDamage(int damage) // Implementing IHealth interface
    {
        ...
        else
        {
            if (!dead)
            {
                ...
                rrb.velocity = Vector2.zero; // Stop any movement
                rrb.gravityScale = 0; // Freeze the boss in place by setting gravity scale to zero
